...
class CrossSectionModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        if ts_days < 1:
            raise ValueError(f"CrossSectionModel requires ts_days >= 1, got {ts_days}")
        if hidden_size < 1:
    ...
    def _forward_full(self, x: torch.Tensor) -> torch.Tensor:
        x_last = x[:, -1]
        x_mean = x.mean(dim=1)
        x_std = x.std(dim=1, unbiased=False) + 1e-6
        h = torch.cat([x_last, x_mean, x_std], dim=-1)
        h = self.input_proj(h)
        h = self.input_norm(h)
    ...
    def _forward_chunked(self, x: torch.Tensor) -> torch.Tensor:
        outputs = []
        for start in range(0, x.shape[2], self.chunk_size):
            end = min(start + self.chunk_size, x.shape[2])
            outputs.append(self._forward_full(x[:, :, start:end]))
...
class ResearchModel:
    ...
    def _read_config(self) -> None:
        ...
        self.lr = float(self.config.get("lr", 5e-6))
        ...
        self.num_attn_layers = int(_cfg(self.config, "num_attn_layers", "numAttnLayers", 2))
        self.num_attn_heads = int(_cfg(self.config, "num_attn_heads", "numAttnHeads", 4))

...
class TCNOnlyModel(nn.Module):
    def __init__(
        ...
        dropout: float = 0.5,
        tcn_channels: int = 384,
        tcn_layers: int = 3,
        tcn_kernel_size: int = 3,
        tcn_dropout: float = 0.45,
        tcn_dilation_base: int = 2,
        num_heads: int = 4,
    ):
        super().__init__()
        if ts_days < 1:
            raise ValueError(f"TCNOnlyModel requires ts_days >= 1, got {ts_days}")
        if tcn_layers < 1:
...
class ResearchModel:
    ...
    def _read_config(self) -> None:
        ...
        self.tcn_channels = int(_cfg(self.config, "tcn_channels", "tcnChannels", 384))
        self.tcn_layers = int(_cfg(self.config, "tcn_layers", "tcnLayers", 3))
        self.tcn_kernel_size = int(_cfg(self.config, "tcn_kernel_size", "tcnKernelSize", 3))
        self.tcn_dropout = float(_cfg(self.config, "tcn_dropout", "tcnDropout", 0.45))
        self.tcn_dilation_base = int(_cfg(self.config, "tcn_dilation_base", "tcnDilationBase", 2))

...
class CrossAttentionBlock(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, dropout: float, ffn_multiplier: int):
        super().__init__()
...
class TransformerHybridModel(nn.Module):
    def __init__(
        ...
        ts_num_layers: int = 2,
        ts_num_heads: int = 4,
        ts_feedforward: int = 512,
        cross_num_layers: int = 1,
        cross_num_heads: int = 4,
        ffn_multiplier: int = 4,
        num_heads: int = 4,
        chunked_inference_threshold: int = 1500,
        chunk_size: int = 512,
    ):
        super().__init__()
        if ts_days < 1:
            raise ValueError(f"TransformerHybridModel requires ts_days >= 1, got {ts_days}")
        if hidden_size % ts_num_heads != 0:
            raise ValueError(f"hidden_size={hidden_size} must be divisible by ts_num_heads={ts_num_heads}")
        if cross_num_layers < 1:
            raise ValueError(f"cross_num_layers must be >= 1, got {cross_num_layers}")
        if num_heads < 1:
        ...
        self.chunked_inference_threshold = int(chunked_inference_threshold)
        self.chunk_size = int(chunk_size)
        self.dropout = nn.Dropout(dropout)

        self.input_proj = nn.Linear(input_size, hidden_size)
        self.temporal_pe = nn.Parameter(torch.zeros(ts_days, hidden_size))
        nn.init.normal_(self.temporal_pe, std=0.02)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=hidden_size,
            nhead=ts_num_heads,
            dim_feedforward=ts_feedforward,
            dropout=dropout,
            activation="gelu",
            batch_first=True,
            norm_first=True,
        )
        self.temporal_encoder = nn.TransformerEncoder(encoder_layer, num_layers=ts_num_layers)
    ...
    def _temporal_features(self, x: torch.Tensor) -> torch.Tensor:
        batch_size, steps, num_stocks, num_features = x.shape
        if steps > self.temporal_pe.shape[0]:
            raise ValueError(f"input T={steps} exceeds configured ts_days={self.temporal_pe.shape[0]}")
        h = x.permute(0, 2, 1, 3).contiguous()
        h = h.view(batch_size * num_stocks, steps, num_features)
        h = self.input_proj(h)
        h = h + self.temporal_pe[:steps].unsqueeze(0)
        h = self.temporal_encoder(h)
        h = h[:, -1]
        return h.view(batch_size, num_stocks, self.hidden_size)

    def _forward_cross_full(self, h: torch.Tensor) -> torch.Tensor:
        ...
        h = F.leaky_relu(h, negative_slope=0.1)
        h = self.dropout(h)
        return self.mlpfc3(h)

    def _forward_cross_chunked(self, h: torch.Tensor) -> torch.Tensor:
        outputs = []
        for start in range(0, h.shape[1], self.chunk_size):
            end = min(start + self.chunk_size, h.shape[1])
            outputs.append(self._forward_cross_full(h[:, start:end]))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x / 4.0
        h = self._temporal_features(x)
        num_stocks = h.shape[1]
        use_chunked = (
            not self.training
            and self.chunked_inference_threshold > 0
            and self.chunk_size > 0
            and num_stocks > self.chunked_inference_threshold
        )
        if use_chunked:
            return self._forward_cross_chunked(h)
        return self._forward_cross_full(h)
...
class ResearchModel:
    ...
    def _read_config(self) -> None:
        ...
        self.hidden_size = int(_cfg(self.config, "hidden_size", "hiddenSize", 256))
        self.fc_size = int(_cfg(self.config, "fc_size", "fcSize", 256))
        self.dropout = float(self.config.get("dropout", 0.4))
        self.lr = float(self.config.get("lr", 3e-6))
        ...
        self.scheduler_step_size = int(self.config.get("scheduler_step_size", 12))
        self.scheduler_gamma = float(self.config.get("scheduler_gamma", 0.5))
        self.grad_clip = float(self.config.get("grad_clip", 5.0))
        self.early_stopping_patience = int(_cfg(self.config, "early_stopping_patience", "earlyStoppingPatience", 5))
        self.seed = int(self.config.get("seed", 42))

        self.ts_num_layers = int(_cfg(self.config, "ts_num_layers", "tsNumLayers", 2))
        self.ts_num_heads = int(_cfg(self.config, "ts_num_heads", "tsNumHeads", 4))
        self.ts_feedforward = int(_cfg(self.config, "ts_feedforward", "tsFeedforward", 512))
        self.cross_num_layers = int(_cfg(self.config, "cross_num_layers", "crossNumLayers", 1))
        self.cross_num_heads = int(_cfg(self.config, "cross_num_heads", "crossNumHeads", 4))
        self.ffn_multiplier = int(_cfg(self.config, "ffn_multiplier", "ffnMultiplier", 4))
        self.num_heads = int(_cfg(self.config, "num_heads", "numHeads", 4))
        self.decorr_lam = float(_cfg(self.config, "decorr_lam", "decorrLam", 0.05))
        self.chunked_inference_threshold = int(
            _cfg(self.config, "chunked_inference_threshold", "chunkedInferenceThreshold", 1500)
        )
        self.chunk_size = int(_cfg(self.config, "chunk_size", "chunkSize", 512))
    ...
    def _init_model(self, trainii: torch.Tensor) -> Model:
        model = Model(
            ...
            ts_num_layers=self.ts_num_layers,
            ts_num_heads=self.ts_num_heads,
            ts_feedforward=self.ts_feedforward,
            cross_num_layers=self.cross_num_layers,
            cross_num_heads=self.cross_num_heads,
            ffn_multiplier=self.ffn_multiplier,
            num_heads=self.num_heads,
            chunked_inference_threshold=self.chunked_inference_threshold,

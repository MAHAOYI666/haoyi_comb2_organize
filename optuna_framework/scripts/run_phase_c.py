...
def main() -> None:
    ...
    candidates = _load_survivors(study_root, args.dry_run, adapter)
    full_segment = study_spec.segment_by_name("full_period")
    if args.dry_run:
        print(f"[DRY-RUN] Phase C study_root={study_root}")
        print(f"[DRY-RUN] candidates={len(candidates)} seeds={list(_phase_seeds(study_spec, adapter))}")
        for candidate in candidates:
            for seed in _candidate_seeds(candidate, study_spec, adapter):
    ...
    results = []
    for candidate in candidates:
        seeds = _candidate_seeds(candidate, study_spec, adapter)
        seed_results = []
        for seed in seeds:
            segment_dir = study_root / "phase_c" / candidate["candidate"] / f"seed_{seed}" / full_segment.name
            run_paths = build_named_run_paths(
            ...
            seeded_model_path = adapter.write_seeded_model(run_paths.segment_dir, study_spec.baseline_config_path, "phase_c")
...
def _load_survivors(study_root: Path, dry_run: bool, adapter: Any) -> list[dict[str, Any]]:
    ...
    return [{"candidate": "candidate_01", "params": adapter.baseline_params(), "seeds": list(adapter.phase_seeds())}]


def _candidate_seeds(candidate: dict[str, Any], study_spec: Any, adapter: Any) -> tuple[int, ...]:
    raw_seeds = candidate.get("seeds")
    if raw_seeds is None:
        raw_seeds = sorted({int(row["seed"]) for row in candidate.get("metrics", []) if "seed" in row})
    if not raw_seeds:
        raw_seeds = _phase_seeds(study_spec, adapter)

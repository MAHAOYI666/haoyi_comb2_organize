...
from comb2_simbase import IndexMask, Memmaper2, ashare_cache_path, daily_label_path, fast, stock_mask_path
from comb2_simbase.benchmark import benchmark_returns_from_cache, load_index_benchmark


REAL_CACHE_VALUE = os.environ.get("COMB2_TEST_CACHE_PATH")
REAL_CACHE = Path(REAL_CACHE_VALUE).expanduser() if REAL_CACHE_VALUE else None
HAS_REAL_CACHE = REAL_CACHE is not None and ashare_cache_path(REAL_CACHE).exists()
...
def test_cache_layout_uses_parent_directory_semantics(tmp_path: Path):
    assert ashare_cache_path(tmp_path) == tmp_path / "AshareCache"
    assert daily_label_path(tmp_path, "vwap30_label1d") == (
        tmp_path / "AshareCache" / "1d_DailyLabel" / "DailyLabel.vwap30_label1d"
    )
    assert stock_mask_path(tmp_path, "StockMask2.LimitMask") == (
        tmp_path / "AshareCache" / "1d_StockMask2" / "StockMask2.LimitMask"
    )
...
def test_index_mask_matches_local_simbase_arrays():
    ...
    simbase_value = os.environ.get("COMB2_TEST_INDEX_MASK_PATH")
    if simbase_value:
        simbase_dir = Path(simbase_value).expanduser()
...
@pytest.mark.skipif(not HAS_REAL_CACHE, reason="COMB2_TEST_CACHE_PATH is unavailable")
def test_memmaper2_reads_real_local_cache():
    path = ashare_cache_path(REAL_CACHE) / "1d_DailyKline" / "DailyKline.close"
...
def test_local_benchmark_returns_match_manual_index_weight_formula():
    ...
    root = ashare_cache_path(REAL_CACHE)
    weights = Memmaper2(root / "1d_IndexWeight" / "IndexWeight.000905.SH").load(dates[0], dates[-1], df_type=True).dloc[:].astype(float)
    close = Memmaper2(root / "1d_DailyKline" / "DailyKline.close").load(dates[0], dates[-1], df_type=True).dloc[:].astype(float)
    prev_close = Memmaper2(root / "1d_DailyKline" / "DailyKline.real_pre_close").load(dates[0], dates[-1], df_type=True).dloc[:].astype(float)
...
def test_local_benchmark_uses_raw_previous_close_instead_of_adjusted_previous_close():
    ...
    weights = Memmaper2(root / "1d_IndexWeight" / "IndexWeight.000905.SH").load(dates[0], dates[-1], df_type=True).dloc[:].astype(float)
    close = Memmaper2(root / "1d_DailyKline" / "DailyKline.close").load(dates[0], dates[-1], df_type=True).dloc[:].astype(float)
    preclose = Memmaper2(root / "1d_DailyKline" / "DailyKline.pre_close").load(dates[0], dates[-1], df_type=True).dloc[:].astype(float)

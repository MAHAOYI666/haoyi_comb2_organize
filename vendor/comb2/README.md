...
训练数据集分块预加载各时点的特征和标签，取样时复用内存快照并执行窗口变换；none/fp4/fp8 直接作用于特征张量。训练存储随日期、时点、股票和特征数量增长，源数据更新后需重建数据集。训练窗口达到 `max_train_days` 后数据集跨训练原位滚动，股票集合变化时在预留列内重排，只读取新增日期。模型保存、加载和 trainii 股票索引回填保持现有接口。
